...
const SparkLine: React.FunctionComponent<SparkLineProps> = ({ className, ratingHistory }) => {
	...
	const peakRating = useMemo(
		() =>
			ratingHistory.reduce(
				(max, historyInstance) =>
					historyInstance.rating.mu > max ? historyInstance.rating.mu : max,
				0
			),
		[ratingHistory]
	)
	const minimumRating = useMemo(
		() =>
			ratingHistory.reduce(
				(min, historyInstance) =>
					historyInstance.rating.mu < min ? historyInstance.rating.mu : min,
				Infinity
			),
		[ratingHistory]
	)
		...
		>
			{ratingHistory.map((contest, idx) => (
				<g
					...
					className={colorFromDelta(
						idx != 0 ? contest.rating.mu - ratingHistory[idx - 1].rating.mu : 0
					)}
				>
					<rect
						stroke="none"
						...
						y={
							chartHeight -
							((contest.rating.mu - floorRating) / (peakRating - floorRating)) * chartHeight
						}
						width={barWidth}
